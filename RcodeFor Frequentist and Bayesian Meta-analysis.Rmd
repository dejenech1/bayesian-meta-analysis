...
print(ma00$summary, digits = 4)

forestplot(ma00, addfit = TRUE, showweights = TRUE)
...
print(ma02$summary, digits = 4)

############################
# Model3. Informative Prior

ma03<-bayesmeta(y = BCGdat[, "yi"],
  sigma = sqrt(BCGdat[, "vi"]),
  mu.prior = c(mean = 0, sd = 4),
  tau.prior = function(x) dhalfcauchy(x, scale = 0.5))
print(ma03$summary, digits = 4)
...
print(ma04$summary, digits = 4)


```

```{r}
forest(ma04, addfit = TRUE, showweights = TRUE)
forest(ma04,addpred=TRUE,addfit=TRUE,showweights=T,
...
ma04$post.interval(tau.level = 0.95)
ma04$post.interval(mu.level = 0.95)

plot(ma04, main = "Posterior Distribution")

ma04$rposterior(n = 10)
...
lines(x, ma02$dposterior(mu = x), col = "red")
lines(x, ma02$dposterior(mu = x, predict = TRUE), col = "blue")
